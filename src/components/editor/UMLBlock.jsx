...
export const UMLBlock = ({
  ...
  useEffect(() => {
    ...
    const renderDiagram = async () => {
      ...
      mermaidIdCounter += 1;
      const svgId = `mermaid-svg-${mermaidIdCounter}`;

      const container = document.createElement('div');
      container.id = `mermaid-wrap-${mermaidIdCounter}`;
      container.style.cssText = 'position:absolute;top:-9999px;left:-9999px;width:600px;opacity:0;pointer-events:none;';
      document.body.appendChild(container);

      try {
        const { svg } = await mermaid.render(svgId, umlCode, container);
        if (!cancelled) {
          setSvgContent(svg);
          setRenderError(null);
        }
  ...
  const eatMouseDown = (e) => { e.preventDefault(); e.stopPropagation(); };
